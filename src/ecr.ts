...
export const runForECR = async (inputs: Inputs): Promise<Outputs> => {
  ...
  const permissionPolicy = inputs.permissionPolicy
  if (permissionPolicy !== undefined) {
    await core.group(
      `Put the permission policy to repository ${inputs.repository}`,
      async () => await putPermissionPolicy(client, inputs.repository, permissionPolicy)
    )
  }
...
const putPermissionPolicy = async (client: ECRClient, repositoryName: string, path: string): Promise<void> => {
  const policyText = await fs.readFile(path, { encoding: 'utf-8' })
  core.debug(`putting the permission policy ${path} to repository ${repositoryName}`)

  await client.send(new SetRepositoryPolicyCommand({ repositoryName, policyText }))
  core.info(`successfully put permission policy ${path} to repository ${repositoryName}`)
}
